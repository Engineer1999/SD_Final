...
def main(wav_path, embedding_per_second=1.0, overlap_rate=0.5):
    ...
    number_of_speakers = len(speakerSlice.items())
    print("Number of speakers : " + str(number_of_speakers))
    ...
    if number_of_speakers > 1:
        number_of_times_multiple_speaker_detected += 1
    '''if number_of_times_multiple_speaker_detected > 3:
        print("Exiting because multiple times more than one person detected!!")
        exit(0)
    p = PlotDiar(map=speakerSlice, wav=wav_path, gui=True, size=(25, 6))
    p.draw()
    path_plot = wav_path + '.png'
    print(path_plot)
    p.plot.savefig(path_plot)
    '''
    #p.plot.show()
